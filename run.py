...
if __name__ == '__main__':
    ...
    if args.is_training == 1:
        for ii in range(args.itr):
            ...
            if args.resume:
                # CORRECTED PATH: Use the 'setting' string to match the saving path
                checkpoint_path = os.path.join(args.checkpoints, setting, 'checkpoint.pth')
                
                if os.path.exists(checkpoint_path):
                    print(f"🔁 Resuming from checkpoint: {checkpoint_path}")
                    # Make sure to load the state dict to the correct device
                    state_dict = torch.load(checkpoint_path, map_location=exp.device)
                    exp.model.load_state_dict(state_dict)
                else:
                    print(f"⚠️ Resume requested but checkpoint not found at {checkpoint_path}. Starting from scratch.")
        ...
        print("Executing is_training=2 block")
        for ii in range(args.itr):
            # setting record of experiments
            setting = '{}_{}_{}_{}_ft{}_sl{}_ll{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_fc{}_eb{}_dt{}_{}_{}'.format(
                args.model_id,
    ...
    # Format the time for better readability (HH:MM:SS)
...
# This is for path management so the script can find other modules.
